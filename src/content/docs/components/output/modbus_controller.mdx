...
- **write_lambda** (*Optional*, [lambda](/automations/templates#config-lambda)):
  ...
  - **payload**: an empty vector to fill; if filled, the return value is ignored and the payload is sent instead. Its element type, and what the bytes mean, depend on `register_type`:
    ...
    - for `register_type: coil`  : `std::vector<uint8_t> &payload` - add the raw frame bytes. This is sent as a
      custom command, so it must be a full Modbus request frame - the leading device-address byte followed by the PDU
      (function code + data); the CRC is added automatically. Unlike
      [`custom_pdu`](/components/modbus_controller/#modbus_custom_pdu), the device-address byte is part of this
      payload.
