...
- **allow_broadcast_read** (*Optional*, boolean): Allow `address: 0` and poll it as if it were a unicast address, for
  a device that doesn't follow the Modbus spec and replies to the broadcast address. Every read this controller
  sends waits for its reply with the normal timeout and retry ladder, and the
