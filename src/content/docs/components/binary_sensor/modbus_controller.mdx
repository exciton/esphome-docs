...
- **response_size** (*Optional*, int): Size of the response for the register in bytes. Defaults to `2`. Only applies to the `holding` and `input` register types; it is ignored for `coil` and `discrete_input`.
